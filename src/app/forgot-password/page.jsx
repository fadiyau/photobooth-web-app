...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setIsSubmitted(true); // Hanya pindah layar jika email terdaftar & sukses terkirim
      } else {
        // Menampilkan pesan error dari API (misal: "Email tidak terdaftar...")
  ...
  return (
    <div className="min-h-screen w-full flex bg-slate-50 items-center justify-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-xl bg-white p-8 sm:p-12 md:p-16 rounded-md border border-gray-100 shadow-xs text-center transition-all">
        ...
        {/* ========================================================= */}
        {/* TAMPILAN 1: SUKSES (EMAIL TERDAFTAR & TERKIRIM)           */}
        {/* ========================================================= */}
        {isSubmitted ? (
          ...
          <>
            ...
            <p className="text-xs sm:text-sm text-gray-500 leading-relaxed mb-6 px-2 sm:px-4">
              There is nothing to worry about, we'll send you a message to help you reset your password.
            </p>

            {errorMsg && (
              <div className="mb-6 text-xs sm:text-sm text-red-600 font-medium bg-red-50 px-4 py-3 rounded-lg border border-red-200 text-left flex items-center gap-2">
                <span>⚠️</span>
                <span>{errorMsg}</span>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5 text-left">
              <div>
                ...
                <input
                  ...
                  className="text-gray-900 w-full px-4 py-3 border border-gray-200 rounded-lg bg-gray-50 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all placeholder:text-gray-400"
                  required
                />
              </div>
